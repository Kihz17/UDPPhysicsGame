...
void Renderer::RenderMeshWithTextures(Ref<Shader> shader, Ref<Mesh> mesh, const std::vector<Ref<SaveableTexture>>& textures, const glm::mat4& transform, float alphaTransparency, bool debugMode)
{
	shader->Bind();
	...
	{
		const Ref<SaveableTexture>& textureData = textures[i];
