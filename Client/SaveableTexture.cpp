...
SaveableTexture::SaveableTexture(Ref<Texture> texture, float ratio, float texCoordScale)
	: texture(texture), ratio(ratio), texCoordScale(texCoordScale)
{

}

void SaveableTexture::Save(YAML::Emitter& emitter) const
{
...
Ref<SaveableTexture> SaveableTexture::StaticLoad(YAML::Node& node)
{
	...
	if (textureType == TextureType::Diffuse)
	{
		...
		return CreateRef<SaveableTexture>(texture, ratio, scale);
	}
	else if (textureType == TextureType::Heightmap)
	{
